...
export function ChangePassWord(url) {
  swal.fire({
...
//status popups
// Allow Outside Click
export function StatusClick(data) {
  console.log(dataService.decodedToken)
  console.log(data)
  if(data.infoApproval != null){
    let html=''
      +'<div class="col-md-12" style="overflow: auto">'
      + '<table class="table table-responsive">'
      +'<thead>'
      +'<tr><td colspan="6">Document No. :- '+data.zoneCode+'</td></tr>'
      +'<tr>'
      +'<th>From</th>'
      +'<th>To</th>'
      ...
      +'</tr>'
      +'</thead><tbody>'
      data.infoApproval.pendingApprovalLevelWise.forEach(info => {
        html+='<tr>'
          +'<td>'+dataService.decodedToken.sub+'</td>'
          +'<td>'
          +'<span class="cellSpan">Pending On Level '+info.level+' for the Below Users:- </span>'
        info.userGroup.forEach((users,key) => {
          html+='<span class="cellSpan">'+(key+1)+'. '+users+'</span>'
        });
        html+='</td>'
          +'<td>'+data.modifiedBy+'</td>'
          +'<td>'+data.modifiedDate+'</td>'
          +'<td>'+data.statusDetail.description+'</td>'
          +'<td></td>'
          +'</tr>'
      });
