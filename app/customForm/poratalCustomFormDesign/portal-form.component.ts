...
export class PortalFormDesign implements OnInit, AfterViewInit {
    dataForm: FormGroup
    ...
    videoTimer = 60
    //mainFiles: any[] = []
    uploadFileMap: Map<string, Blob[]> = new Map();
    ...
    ngOnInit() {
        ...
        this.actionList["add"].buttonList.forEach(btn => {
            console.log(btn)
    ...
    getFormFields() {
        ...
        */
        ...
        if (this.params.action == "edit" || this.params.action == "view") {
            ...
            this.layoutService.toggleCandidateApplyScreen(true);
        }
    }
    ...
    fileChangeEventMain(doc) {
        ...
        if (doc.target.files && doc.target.files.length > 0) {
            if (!this.uploadFileMap.has('files'))
                this.uploadFileMap.set('files', []);
            for (const file of doc.target.files) {
                this.uploadFileMap.get('files').push(file);
            }
        }
    }
    ...
    getOptions(fields, value, object, parentFieldId, level) {
        ...
        // console.log(fields)
        environment.api_url = environment.login_url + "/module";
        this.appService.getById(fields.srcModule, 120,30).subscribe(module => {
            // console.log(module)
    ...
    getData() {
        ...
        this.appService.getById(this.params.data, this.params.moduleId,31).subscribe(data => {
            console.log(data)
            this.formData = data
            console.log(this.formData)
            this.setDataValues();
        ...
        this.fieldList.forEach(fields => {
            if (fields.fieldType != 'table' && fields.fieldType != 'object' && fields.fieldType != 'Section' && fields.fieldType != 'multipleCheck' && fields.entryType != 'UDF' && fields.fieldType != 'file') {
                ...
                if (fields.fieldName == 'cityId') {
                    ...
                    this.appService.getById(this.formData[fields.fieldName], fields.srcModule,32).subscribe(data => {
                        console.log(data);
            ...
            if (fields.fieldType == 'table') {
                ...
                if (fields.srcModule != null) {
                    ...
                    if(this.formData.jobPostingId){
                        ...
                        this.appService.getById(this.formData.jobPostingId,fields.srcModule,33).subscribe(posting=>{
                            this.videoTimer = posting["introVideoDuration"];
                            this.fileName = this.formData["id"]+" - "+new Date().toDateString()
                            console.log(posting)
                            ...
                            this.childTableForms[fields.fieldName].controls.forEach(ft => {
                                    ...
                                    fields.childCustomFields.forEach(ch => {
                                        if (ch.fieldType == 'select' || ch.fieldType == 'autoCompleter' || ch.fieldType == 'multiSelect') {
                                            switch (ch.fieldName) {
                                                case 'cityId':
                                                    environment.api_url = environment.login_url + "/city";
                                                    this.appService.getById(childData[ch.fieldName], ch.srcModule,34).subscribe(data => {
                                                        let city = data
                                                        console.log(city)
        ...
        if (event != "") {
            switch (event) {
                ...
                case 'Submit':
                    ...
                    break;
                case 'Start Test':
                    this.router.navigate(['assessment-test',{params: this.params.token}] , {  relativeTo: this.route.parent, skipLocationChange: true });
    ...
    OnSave(event) {
        ...
        switch (this.moduleAction) {
            case 'add':
                ...
                if(this.uploadFileMap.size > 0) {
                    formData["uploadFileMap"] = this.uploadFileMap;
                }
                if(this.link != ""){
    ...
    setLink(recordedVideo) {
        // console.log(event)
        // this.link = event
        recordedVideo.name = `${this.formData['id']}-${this.formData['name']}- ${new Date().toDateString()}`;
        recordedVideo.lastModifiedDate = new Date();
        this.uploadFileMap.set('introVideoFile',[recordedVideo]);
    }
}
