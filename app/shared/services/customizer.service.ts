...
export class CustomizerService {
  ...
  lightDarkLayoutSolidTransBGColors: any = [];
  ...
  getData() {
    ...
    this.lightDarkLayoutSolidTransBGColors = this.getlightDarkLayoutSolidTransBGColors();
  ...
  getlightDarkLayoutSolidTransBGColors() {
    return this.light_dark_colors.filter(_ => _.type === 'trans')
      .map(color => {
        color.active = (color.code === this.config.templateConf.layout.sidebar.backgroundColor);
        return { ...color };
      });
  ...
  switchLayout(layout: string, isBgImageDisplay: boolean) {
    ...
    if(layout.toLowerCase() === 'light') {
      ...
      conf.layout.sidebar.backgroundColor = this.light_dark_colors[15].code;
