...
@Component({
    ...
    selector: 'app-custom-wizard-form',
    templateUrl: './custom-wizard-form.component.html',
    styleUrls: ['./custom-wizard-form.component.scss']
})
export class CustomWizardForm implements OnInit {
    dataForm: FormGroup
    ...
    documents: any[] = []
    wizardStepsFields: customFields[] = [];
    ...
    ngOnInit(){
        ...
        this.actionList["add"].buttonList.forEach(btn => {
            // console.log(btn)
            if(btn["button"]["purpose"] == 'form' && btn["button"]["type"] == "btn" && this.params.action){
                let act = {}
        ...
        this.wizardStepsFields = this.fieldList.filter(field => field.fieldType == "wizardStep");
        console.log(this.wizardStepsFields);
    ...
    getFormFields(){
        ...
        this.fieldList.forEach(field=>{
            ...
            if(field.fieldType != 'section' && field.fieldType != 'wizardStep' && field.fieldType != 'table' && field.fieldType != 'object'){
                let value: any;
    ...
    getOptions(fields,value, object, parentFieldId,level){
        ...
        // console.log(fields)
        environment.api_url = environment.login_url+"/module";
        this.appService.getById(fields.srcModule,120,40).subscribe(module=>{
            console.log(module)
    ...
    getData(){
        ...
        this.appService.getById(this.params.data,this.params.moduleId,41).subscribe(data=>{
            console.log(data)
            this.formData = data
            console.log(this.formData)
            this.setDataValues();
        ...
        this.fieldList.forEach(fields=>{
            ...
            if(fields.fieldType != 'table' && fields.fieldType != 'object' && fields.fieldType != 'Section' && fields.fieldType != 'multipleCheck' && fields.entryType != 'UDF'){
                ...
                if(fields.fieldName == 'cityId'){
                    ...
                    this.appService.getById(this.formData[fields.fieldName],fields.srcModule,42).subscribe(data=>{
                        console.log(data);
                                ...
                                fields.childCustomFields.forEach(ch => {
                                    if(ch.fieldType == 'select' || ch.fieldType == 'autoCompleter' || ch.fieldType == 'multiSelect'){
                                        switch(ch.fieldName){
                                            case 'cityId':
                                                environment.api_url = environment.login_url+"/city";
                                                this.appService.getById(childData[ch.fieldName],ch.srcModule,43).subscribe(data=>{
                                                    let city = data
                                                    console.log(city)
    ...
    getValidation(fieldList){
        fieldList.forEach(field => {
            if(field.fieldType != 'section' && field.fieldType != 'table' && field.fieldType != 'wizardStep' && field.fieldType != 'object'){
                let validations = []
